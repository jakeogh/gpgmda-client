        ...
        if 'exists' not in line:
            if 'gpgMaildir' in line:
                if line.startswith('>f'):
                    new_gpgmda_file_path = gpgMaildir_archive_folder / Path(line.split(' ')[1])
    ...
    if stdout:
        ...
        if len(gpg_cmd_proc_output_stdout) > 0:
            ...
            if stats.st_size <= 1668:
                ...
                search_server_logs_command = ' '.join(['echo', '"' + 'zgrep', '--', gpgfile.name, '/var/log/*' + '"'])
...
def gpgmaildir_to_maildir(*,
    ...
    if gpgmaildir_file_count != maildir_file_count:  # not a good check.
        ...
        files_in_gpgmaildir = [path for path in files_pathlib(gpgmaildir, verbose=verbose,)]
        #assert isinstance(files_in_gpgmaildir[0], Path)
        files_in_maildir = [path for path in files_pathlib(maildir, verbose=verbose,)]
